...
let grupoCorridas = L.layerGroup().addTo(map);
...
onAuthStateChanged(auth, (user) => {
  ...
  iniciarGPS();
  carregarCorridas();
  carregarCorridaAtiva();   // aceita

});

function carregarCorridaAtiva() {
...
function iniciarGPS() {
  ...
  navigator.geolocation.watchPosition(async (pos) => {
    ...
    if (!marcadorMotorista) {
      ...
      // Continua seguindo o motorista
      if (seguirMotorista) {
        map.panTo([minhaLat, minhaLng]);
      }

    }

    // Atualiza a rota, se existir corrida
    if (corridaAtual) {
      desenharRota();
    }

    // Atualiza localização no Firebase
...
<p>${corrida.origemEndereco}</p>

<hr>

<p><b>🏁 Destino</b></p>

<p>${corrida.destinoEndereco}</p>
...
btnFinalizar.onclick = async () => {
    ...
    const confirmar = confirm("Deseja finalizar esta corrida?");

    if(!confirmar) return;

    await updateDoc(doc(db,"corridas",corridaAtual.id),{
    ...
    if(marcadorPassageiro){
        map.removeLayer(marcadorPassageiro);
    }
